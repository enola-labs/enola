...
- **Import-closure** ([`internal/explainers/importclosure`](internal/explainers/importclosure/importclosure.go)) — reports what importing a Python package actually executes. It reads a graph of its own, built at FILE granularity from the same dependency facts, because the module graph every other explainer uses resolves both ends of an edge up to the enclosing package and so cannot say which modules beneath one were paid for. Imports that do not run at import time are excluded on the extractor's `deferred` prop; the package `__init__.py` files Python executes implicitly are added, since no import statement names them. Per top-level package it emits one **informational** summary — modules loaded, share of the package, depth, third-party packages on the path — and, at `0.7`, the package initialisations that **dominate** the closure: the modules reachable only through that `__init__.py`, which is what splitting it would return. Dominance rather than depth is the discriminator because depth does not separate a barrel worth splitting from ordinary layering — on a real package 31 of 36 third-party arrivals sit three or more hops in, and most are core dependencies that legitimately load. Its graph is read by no coupling explainer: a package node accumulates an edge from every file beneath it, so folding import-time reachability into the module graph would make each namespace package the most-depended-upon node in the repository.
**How many findings an explainer reports.** One explainer reporting a large repository a finding at a time buries every other: `dead-methods` produces 955 on a large Rails application, and the performance analyzer produced 1,254 of enola's own 1,385 before it was capped. So an explainer reports at most [`common.MaxIndividualInsights`](internal/explainers/common/cap.go) individually and the rest as one rollup; the tool behind it still returns everything, and the ranking spent on that budget is the explainer's own (`orphans` spends it on the most actionable candidates, `performance` on the most severe). The cap costs diff visibility — [`internal/diff`](internal/diff/diff.go) identifies a finding by explainer and title, so one past the cap reaches a diff only as the rollup's number — which is why the proof-class explainers (`cycles`, declared `layers`, `intent`, `constraints`) are never capped, and why `dead-code`, `performance` and `package-metrics` each annotate the facts they are about as a second channel. `vendored-candidates` is uncapped in the other direction and says why at its own entry.

- **Package-metrics** ([`internal/metrics`](internal/metrics/explainer.go)) — computes Robert C. Martin's package metrics per module from the import graph and the symbol kinds: afferent coupling (Ca), efferent coupling (Ce), instability (I = Ce/(Ca+Ce)), abstractness (A, the share of a module's symbols that are interfaces or abstract types), and distance from the main sequence (D = |A+I-1|). It emits findings for modules off the main sequence at `D > 0.7` and for the most depended-upon module. The metrics are arithmetic over edges the graph already holds, so they are exact; what is inferred is that a large D is a problem, which is why the findings sit below `1.0`. It overlaps the `hotspots` explainer by design and neither defers to the other: hotspots reports the blast radius through a module, this reports what kind of module it is, and the two are read together.
- **Dead-code** ([`internal/orphans`](internal/orphans/explainer.go)) — reports symbols nothing in the snapshot references, in every language the extractors cover. Two classes: `isolated` (no references in or out) and `unreferenced` (nothing calls it, but it still calls others). Three confidence tiers, and they encode what the graph can actually see: `high` for functions, whose plain calls are reliably tracked; `medium` for structs, interfaces and types, whose use is not edge-tracked; `low` for everything else. Exported symbols are reported but flagged, since a consumer may sit outside the snapshot. Its reference index folds in usage that has no enclosing symbol — C and C++ file-scope registration macros (`module_init`, `EXPORT_SYMBOL`), calls inside `#define` bodies, token-pasted attribute callbacks — and function-pointer field assignments. Still invisible: macros from system headers, `#ifdef` variants, and macro-wrapped struct definitions. It stays silent on any symbol `dead-methods` has claimed, so the scoped Ruby answer is not restated from a weaker signal.
- **Performance** ([`internal/perf`](internal/perf/explainer.go)) — estimates per-function algorithmic complexity and ranks the risks, from parser-derived facts rather than inference: `cyclomatic`, `loop_depth`, `scaling_loop_depth`, `loop_count`, `calls_in_loop`, `performs_io` and `recursive_self`, all recorded by the extractors. Four finding kinds: `nested-loop` (loops nested inside one function), `compounded` (nesting that grows across the call graph), `call-in-loop` (an I/O, DB or network call per iteration, the N+1 shape), and `recursion` (direct or mutual, via a local Tarjan SCC over the `calls` graph). The Big-O label is a deterministic function of those props and is an estimate of structural worst case, never a proof. Each language models its own idea of a loop, and five ecosystems (Swift, Kotlin/Java, TypeScript/JavaScript, Dart/Flutter, Rust) bypass the generic keyword gate entirely because their idioms reuse those verbs for in-memory work. Rust is the starkest: an async runtime's whole vocabulary is `send`, `recv`, `read`, `write` and `poll` over channels and buffers that never leave the process, and on tokio the generic gate produced 42 high-severity findings against 3 after the handler. **Scala looks like a sixth and measurably is not**, which is worth stating because the absence otherwise reads as an oversight somebody should fix: its concurrency primitives are spelled `update`, `get` and `set`, exactly the shape that earned the other five a handler, but the most effect-heavy application in the benchmark corpus produces five findings and no high-severity ones. Those calls sit inside `for … yield` and flatMap chains that the Scala loop model already discounts, because Scala spells iteration and effect-sequencing identically and the ambiguous forms raise `loop_depth` but not `scaling_loop_depth`. Re-running the analyzer over an effect-heavy Scala application is how to check that still holds. Each language's loop model — which constructs count as iteration, and which callbacks are deferred rather than per-iteration — is documented per language in [docs/extraction/](docs/extraction/), because the props come from the extractors rather than from here. Its `call-in-loop` findings stay silent on any symbol `query-loops` has claimed; the other three kinds are questions that explainer does not ask, and are unaffected.
...
`--help`, `--list` and `--status` are served from two public packages, for the same reason as `pkg/explain`: they are text and data with no engine underneath, so anything that wants only `--help` can reach it without pulling in the engine.
...
They live in a package rather than in `cmd/enola` because they are built on `internal/` packages — baseline resolution, the hook heartbeat, the file lock — and gathering them behind one exported surface keeps the gate's exit-code contract and the hook's silence rules in a single place rather than restated at each call site. Deliberately *not* folded into `pkg/cli`, which is pure text: anything wanting only `--help` would otherwise pull in the whole engine.
...
`newEngine` builds every engine these commands construct. They build their own — a gate has to snapshot the tree it is grading — so every command that needs one must route through that single function; constructing `bootstrap.NewEngine` directly at a call site is how the same binary comes to produce different snapshots depending on which command was invoked.

**[`pkg/cli`](pkg/cli)** renders what a binary prints about itself.

- `OSSTools()` is the `--list` catalogue: name plus a one-line summary. It is hand-written on purpose — the descriptions registered with `mcp.AddTool` are multi-paragraph agent prompts, unusable in a terminal. `TestE2E_ToolCatalogueMatchesRegisteredTools` ([`internal/server/e2e_test.go`](internal/server/e2e_test.go)) asserts set equality against the tools the running server actually registers, so the two cannot drift. `RenderToolList()` renders it.
- `DefaultHelp(Binary)` returns the `HelpSpec` shared by every enola binary — usage, flags, config path, examples, MCP configuration, build — with the binary's name, command package and version ldflag substituted in. `Commands`, `Flags` and `Sections` are plain data, so a caller composing its own help builds the spec and hands it to `RenderHelp`. `RenderHelp` does the layout (a 22-column description gutter, with continuation lines aligned to it).
...
**Template cloning.** Every server gets its own `Clone` of the base template. `html/template` refuses to clone a template that has already executed, so rendering straight from the package-level base would break the *next* dashboard's construction.
...
Every figure derives from one measurement: **the token size of the source enola actually parsed**. Extraction records it as `Meta.SourceBytes`, the on-disk size of the `FilesParsed` set; [`internal/server/autocluster.go`](internal/server/autocluster.go) converts it at `charsPerToken` and records it with the snapshot call, and it is cached per repo in the usage file.
...
A tool with no entry in the table falls through to `defaultWeight` rather than scoring zero, so an unpriced tool is never counted as worthless. The map is written once at package init and never after, so the concurrent readers on the tool callback need no lock.
...
enola is a stdio [MCP](https://modelcontextprotocol.io/) server. It exposes **twenty-two tools** and no MCP resources — everything flows through tool calls. The tools defined in [`internal/server/server.go`](internal/server/server.go) are listed below, each leading with the question it answers.
...
It is its own package because `internal/server` cannot import `pkg/bootstrap`, which imports the server. A leaf depending on `engine` and `diff`, depended on by both, is the only shape with no cycle — and one implementation is the point, since two tools disagreeing about what "stale" means is itself a kind of drift. It re-hashes the repo, so it belongs at a deliberate decision point rather than on every tool call.
...
**Owned files wherever the tool allows one.** Claude Code loads `.claude/rules/*.md` at launch with the same priority as a project `CLAUDE.md`; Cursor has `.cursor/rules/`; Copilot has `.github/instructions/*.instructions.md`. In each case enola writes **a file it owns outright** rather than performing surgery on a document the user hand-maintains, and so do opencode's and Pi's targets — so for five of the seven targets the destructive-edit risk cannot arise at all. Only `AGENTS.md` (and the user-level `AGENTS.md` files) are genuinely shared, so only they need the sentinel machinery — and the repo-root one is never *created*, since a tool that drops a new instruction file into a repository uninvited is one that gets removed.
...
**Codex, Copilot and Pi all read the repository's `AGENTS.md`.** So locally they need no instruction file of their own, and enola deliberately writes none — a second repo-local copy would put the same instruction into the same context window twice. What they do need is their *user-level* files (`~/.codex/AGENTS.md`, `~/.pi/agent/AGENTS.md`), which reach projects where nobody has run `enola install`. Those are written only when the tool's own config directory already exists: that directory is the evidence the tool is installed, and creating `~/.codex` for someone who does not use Codex would be littering in a home directory to no purpose.

**Pi is the exception that still gets a local file,** because Pi has no MCP client: an instruction naming enola's tools would name tools Pi cannot call. [`pkg/install/pi_extension.js`](pkg/install/pi_extension.js) is embedded in the binary and written to `.pi/extensions/enola.js` (or `~/.pi/agent/extensions/enola.js`). It is a minimal MCP client over stdio: it starts the enola server with the absolute path of the binary that installed it, registers each tool from `tools/list` as `enola_<tool>` with its schema passed through (plus an empty `properties` where a no-argument tool omits it, which OpenAI-compatible endpoints otherwise reject along with the whole request), and names those tools in the system prompt only while the server is up. With `--hooks` it also calls `enola hook session-start` and `enola hook stop` from Pi's `session_start` and `agent_end` events, with the same stdin payload Claude Code sends. A stop report is sent back as a message that starts one more turn, and the `agent_end` closing that turn carries `stop_hook_active`, so the hook's own loop breaker applies unchanged.

It is JavaScript although Pi loads TypeScript too, because the file sits inside the repository and the TypeScript extractor detects a repository by any `.ts` file: a `.ts` extension made a Go repository detect as TypeScript, changed its extractor set, and left every earlier baseline ungradable. For the same reason every file `enola install` owns is ignored by every config ([`config.InstalledFiles`](internal/config/config.go)), appended in `Normalize` beside the output directory so a config's own `ignore:` list cannot drop them.
...
**`enola hook stop`** ([`pkg/command/hook.go`](pkg/command/hook.go)) is what the installed hook invokes. It grades the session's change and emits the verdict as `additionalContext`, but only when a baseline was pinned *and* the change either regressed, introduced a finding no policy enforces, or left a baseline the gate refuses to compare. Three rules govern it:
...
3. **It speaks once per distinct report per session.** On Stop, `additionalContext` is not an annotation on a finished turn: it *prevents the turn from ending*. The harness feeds the text back to the model and stops again afterwards, setting `stop_hook_active` on that second Stop to say so. A hook that re-grades there re-emits the identical verdict from an unchanged baseline and buys another model turn, repeating until the harness's consecutive-block cap overrides it, so the session ends on a warning rather than on the report. So the hook returns silently whenever `stop_hook_active` is set, before grading anything, and `hookstate` remembers what was said (keyed by [`check.Verdict.ReportKey()`](pkg/check/render.go) and the session id) so a repeat inside one session stays unsaid while a new session, a different finding, or a recurrence after a clean grade is heard again.

The first two rules were in place from the start; the third was learned from [#288](https://github.com/enola-labs/enola/issues/288), where a comment in `hook.go` asserted that `additionalContext` handed the model something "without preventing it from finishing". That was never true and never checked. The durable defence is not the flag but the test that counts: `TestStopHook_FiresInARealSession` ends a real session and now asserts the report was emitted exactly once, which is the same discipline that caught the Stop hook never firing at all.
...
### `package_metrics` — "what kind of package is this?"

Robert C. Martin / JDepend metrics per module: afferent coupling (Ca), efferent coupling (Ce), instability (I), abstractness (A) and distance from the main sequence (D). Filters by repo and package, sorts by any metric, and rides the usual ladder: `summary` (aggregate health) → `compact` (table) → `full` (JSON), plus `max_tokens`.

### `find_orphans` — "what does nothing reference?"

Symbols with no incoming references, in every language the extractors cover, split into `isolated` and `unreferenced` and tiered `high` / `medium` / `low` by how reliably the graph tracks that kind of use. `summary` (counts by tier) → `compact` → `full` (per-symbol `file:line`), plus `max_tokens`. Candidates to verify, never a verdict: an exported symbol may have a consumer outside the snapshot.

### `analyze_performance` — "where does cost grow with the input?"

Per-function Big-O estimates and ranked risks over four kinds — `nested-loop`, `compounded`, `call-in-loop` and `recursion` — computed from extractor props rather than inferred. Filters by package, repo and symbol, gates on `min_severity`, takes `limit`, and rides the same ladder plus `max_tokens`.
